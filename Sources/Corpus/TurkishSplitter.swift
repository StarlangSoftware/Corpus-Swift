...
class TurkishSplitter : SentenceSplitter{
    ...
    */
    private func __isNextCharUpperCaseOrDigit(line: String, i: Int) -> Bool{
        ...
        if j == line.count || (TurkishLanguage.UPPERCASE_LETTERS + TurkishLanguage.DIGITS + "-").contains(Word.charAt(s: line, i: j)){
            return true
        } else {
            return false
        }
    ...
    */
    private func __isPreviousWordUpperCase(line: String, i: Int) -> Bool{
        var j : Int = i
        while j >= 0 && (Word.charAt(s: line, i: j) == " " || (TurkishLanguage.LOWERCASE_LETTERS + "qxw").contains(Word.charAt(s: line, i: j))){
            j = j - 1
        }
        if j == -1 || (TurkishLanguage.UPPERCASE_LETTERS + "QWX").contains(Word.charAt(s: line, i: j)){
            return true
        } else {
            return false
        }
    ...
    */
    private func __isNextCharUpperCase(line: String, i: Int) -> Bool{
        ...
        if j == line.count || (TurkishLanguage.UPPERCASE_LETTERS + "\"\'").contains(Word.charAt(s: line, i: j)){
            return true
        } else {
            return false
        }
    ...
    */
    private func __isNameShortcut(currentWord: String) -> Bool{
        if currentWord.count == 1 && TurkishLanguage.UPPERCASE_LETTERS.contains(currentWord){
            return true
        }
        if currentWord.count == 3 && Word.charAt(s: currentWord, i: 1) == "." && TurkishLanguage.UPPERCASE_LETTERS.contains(Word.charAt(s: currentWord, i: 2)){
            return true
        }
        return false
    }
    ...
    */
    private func __numberExistsBeforeAndAfter(line: String, i: Int) -> Bool{
        if i + 1 < line.count && i > 0{
            let previousChar = Word.charAt(s: line, i: i - 1)
            let nextChar = Word.charAt(s: line, i: i + 1)
            return TurkishLanguage.DIGITS.contains(previousChar) && TurkishLanguage.DIGITS.contains(nextChar)
        } else {
            return false
        }
    }
    ...
    */
    private func __isTime(line: String, i: Int) -> Bool{
        if i + 2 < line.count{
            ...
            return TurkishLanguage.DIGITS.contains(previousChar) && TurkishLanguage.DIGITS.contains(nextChar) && TurkishLanguage.DIGITS.contains(twoNextChar)
        } else {
            return false
        }
    }
    ...
    */
    public override func split(line: String) -> [Sentence]{
        ...
        while i < line.count{
            if SentenceSplitter.SEPARATORS.contains(Word.charAt(s: line, i: i)){
                ...
                if SentenceSplitter.SENTENCE_ENDERS.contains(Word.charAt(s: line, i: i)){
                    ...
                    if Word.charAt(s: line, i: i) == "." && currentWord != "" && (webMode || emailMode || TurkishLanguage.DIGITS.contains(Word.charAt(s: line, i: i - 1))){
                        currentWord = currentWord + String(Word.charAt(s: line, i: i))
                    } else {
